...
import { useRegisterMutation } from "../lib/user/userApi.js";
import Loading from "./Loading.jsx";

export default function () {
    const { register, handleSubmit } = useForm();
    const [credentials, { isLoading, }] = useRegisterMutation();
    const dispatch = useDispatch();
    const router = useRouter();

    const submitHandler = async (data) => {
        try {
            const result = await credentials(data).unwrap();
            if (result) {
                router.push("/");
            ...
            dispatch(loginError())
            console.log(err)
            toast.error(err?.data?.message || "Can't connect to server!");
        }
    };
